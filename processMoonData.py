...
earthCart = np.array([361000.0, 0.0, -42100.0])

#method to calculate azimuth angle
#incomplete - need lat & long of earth from lunar fixed frame?
def calc_azimuth(lat, long):
    #this is what we need to figure out (the latitude and longitude of earth from the moon) in radians
...
f = open("smallestmoondata.txt", "w")

with open('smallmoondata.csv', 'r') as read_obj:
